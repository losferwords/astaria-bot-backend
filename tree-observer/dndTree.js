// Get JSON data
treeJSON = d3.json('mcts-trees/9a3c9fa0-000a-11ec-bd64-cf5f68c4e71c-1629316809761.json', function (error, treeData) {
  ...
  function update(source) {
    ...
    nodeEnter
      .append('circle')
      .attr('class', (d) => {
        return 'nodeCircle ' + (d.hero ? d.hero : '');
      })
      .attr('r', 0)
      .style('fill', (d) => {
    ...
    nodeUpdate.select('text.nodeTextInfo').style('fill-opacity', 1);
    ...
    nodeExit.select('text.nodeTextInfo').style('fill-opacity', 0);

    // Update the links…
    var link = svgGroup.selectAll('path.link').data(links, (d) => {
      return d.target.id;
    });
